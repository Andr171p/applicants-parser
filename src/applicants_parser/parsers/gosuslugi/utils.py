...
from ...constants import GOSUSLUGI_URL, GOSUSLUGI_SEARCH_URL

logger = logging.getLogger(__name__)


async def asearch_universities(async_browser: AsyncBrowser, query: str) -> list[str]:
    logger.info(f"---SEARCH UNIVERSITIES BY QUERY {query}---")
    ...
    logger.info(f"---FOUND {len(university_urls)} UNIVERSITIES---")
